...
You can verify any **Diamonds** bet in three ways. All of them run **the same math the server used** —
nothing is trusted, everything is recomputed.

## What you need

From your Diamonds bet history, each bet shows:
...
[`paytable.json`](https://github.com/maczo-co/originals-diamonds/blob/main/paytable.json); the verifier
...
```
https://verify.maczo.co/originals/?game=diamonds&server=<revealed>&commit=<hash>&client=<you>&nonce=<n>&params=<url-encoded JSON>
```

## Method 2 — Browser, manual

1. Open https://verify.maczo.co/originals/?game=diamonds (works fully offline; view source).
...
**No player inputs.** The outcome is drawn purely from the seed — `params` is `{}` (or omitted).

## Method 3 — Node.js CLI

```bash
git clone https://github.com/maczo-co/originals-diamonds
cd originals-diamonds
